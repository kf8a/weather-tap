...
func five_minute_observations(db *sqlx.DB, c *gin.Context) {
	...
	rows, err := db.Queryx("select * from (select air_temp107_avg, relative_humidity_avg, leaf_wetness_mv_avg, solar_radiation_avg, wind_direction_d1_wvt, wind_speed_wvt, reportPCP * 25.4 as rain_mm, lter_five_minute_a.datetime from weather.lter_five_minute_a left join weather.raingauge on raingauge.datetime = lter_five_minute_a.datetime order by datetime desc limit ) t1 order by datetime", limit(c, 1154))
